...
{
    public class AndAlsoFormulaRef : AndAlsoFormula, IAndAlsoFormulaRef
    {
        protected internal AndAlsoFormulaRef()
            : this(default(IFormulaRef), default(IFormulaRef), default(ITypeDeclaration), default(IMethodDeclaration), default(IFormulaRef))
        {
        }

        protected internal AndAlsoFormulaRef(IFormulaRef left, IFormulaRef right, ITypeDeclaration type, IMethodDeclaration method, IFormulaRef parent)
        ...
        public IAndAlsoFormula Establish()
        {
            var established = default(IAndAlsoFormula);
            FirstHierarchyOnly(() =>
            {
                ...
                established = Formula.AndAlso(left, right, type, method, parent);
            });
            return established;
        }
